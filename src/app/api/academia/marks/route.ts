...
import { getMarks } from 'reddy-api-srm';
...
export async function GET(req: Request) {
  try {
    ...
    const data = await getMarks(cookie) as any;
    
    if (data && data.error) {
      console.error('[API Marks Error] SRM backend returned an error:', data);
      return NextResponse.json({ 
        error: data.error || 'Failed to fetch marks from SRM',
    ...
    console.error('[API Marks Exception]', error);
