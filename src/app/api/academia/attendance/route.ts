...
import { getAttendance } from 'reddy-api-srm';
...
export async function GET(req: Request) {
  try {
    ...
    const data = await getAttendance(cookie) as any;
    
    if (data && data.error) {
      console.error('[API Attendance Error] SRM backend returned an error:', data);
      return NextResponse.json({ 
        error: data.error || 'Failed to fetch attendance from SRM',
    ...
    console.error('[API Attendance Exception]', error);
